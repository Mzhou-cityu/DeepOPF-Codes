...
def total_branch8(m=None,mpc=None,initial=None):
    global Lower_bound
    global Upper_bound
...
#            m.Equation(pd[i] == 0)
    ...
    for i in range(0,len(Load_index_list)):
        pd_temp38[i]=Input[i].value[0]
        
    pd_save8.append(pd_temp38)

        
    # print(Slack_Pg_violation[0].value)
    print(compare_temp[len(branch_list)+1].value[0])
    
    FINAL8.append(compare_temp[len(branch_list)+1].value[0])
    # Final.append(compare_temp[len(branch_list)-1].value[0])
    solver8=time.time()-start
    print(solver8)
    solver_time8.append(solver8)
    
    filenamesolver8= dir+'s-solvertime8.txt'
    fidsolver8=open(filenamesolver8,'at')
    fidsolver8.write("%9.4f;" % (solver8))
    fidsolver8.write('\n')
    fidsolver8.close()
    
    filenamesolution8= dir+'s-total30_all-8.txt'
    fidsolution8=open(filenamesolution8,'at')
    fidsolution8.write("%9.4f;" % (compare_temp[len(branch_list)+1].value[0]))
    fidsolution8.write('\n')
    fidsolution8.close()
    
def total_branch9(m=None,mpc=None,initial=None):
    global Lower_bound
    global Upper_bound
...
#            m.Equation(pd[i] == 0)
    ...
    #     pd_temp2[i]=pd[i].value[0]
    for i in range(0,len(Load_index_list)):
        pd_temp39[i]=Input[i].value[0]
        
    pd_save9.append(pd_temp39)

        
    # print(Slack_Pg_violation[0].value)
    print(compare_temp[len(branch_list)+1].value[0])
    
    FINAL9.append(compare_temp[len(branch_list)+1].value[0])
    # Final.append(compare_temp[len(branch_list)-1].value[0])
    solver9=time.time()-start
    print(solver9)
    solver_time9.append(solver9)
    
    filenamesolver9= dir+'s-solvertime9.txt'
    fidsolver9=open(filenamesolver9,'at')
    fidsolver9.write("%9.4f;" % (solver9))
    fidsolver9.write('\n')
    fidsolver9.close()
    
    filenamesolution9= dir+'s-total30_all-9.txt'
    fidsolution9=open(filenamesolution9,'at')
    fidsolution9.write("%9.4f;" % (compare_temp[len(branch_list)+1].value[0]))
    fidsolution9.write('\n')
    fidsolution9.close()
    
def total_branch10(m=None,mpc=None,initial=None):
    global Lower_bound
    global Upper_bound
...
#            m.Equation(pd[i] == 0)
    ...
    #     pd_temp2[i]=pd[i].value[0]
    for i in range(0,len(Load_index_list)):
        pd_temp310[i]=Input[i].value[0]
        
    pd_save10.append(pd_temp310)

        
    # print(Slack_Pg_violation[0].value)
    print(compare_temp[len(branch_list)+1].value[0])
    
    FINAL10.append(compare_temp[len(branch_list)+1].value[0])
    # Final.append(compare_temp[len(branch_list)-1].value[0])
    solver10=time.time()-start
    print(solver10)
    solver_time10.append(solver10)
    
    filenamesolver10= dir+'s-solvertime10.txt'
    fidsolver10=open(filenamesolver10,'at')
    fidsolver10.write("%9.4f;" % (solver10))
    fidsolver10.write('\n')
    fidsolver10.close()
    
    filenamesolution10= dir+'s-total30_all-10.txt'
    fidsolution10=open(filenamesolution10,'at')
    fidsolution10.write("%9.4f;" % (compare_temp[len(branch_list)+1].value[0]))
    fidsolution10.write('\n')
    fidsolution10.close()
    
def total_branch11(m=None,mpc=None,initial=None):
    global Lower_bound
    global Upper_bound
...
#            m.Equation(pd[i] == 0)
    ...
    #     pd_temp2[i]=pd[i].value[0]
    for i in range(0,len(Load_index_list)):
        pd_temp311[i]=Input[i].value[0]
        
    pd_save11.append(pd_temp311)

        
    # print(Slack_Pg_violation[0].value)
    print(compare_temp[len(branch_list)+1].value[0])
    
    FINAL11.append(compare_temp[len(branch_list)+1].value[0])
    # Final.append(compare_temp[len(branch_list)-1].value[0])
    solver11=time.time()-start
    print(solver11)
    solver_time11.append(solver11)
    
    filenamesolver11= dir+'s-solvertime11.txt'
    fidsolver11=open(filenamesolver11,'at')
    fidsolver11.write("%9.4f;" % (solver11))
    fidsolver11.write('\n')
    fidsolver11.close()
    
    filenamesolution11= dir+'s-total30_all-11.txt'
    fidsolution11=open(filenamesolution11,'at')
    fidsolution11.write("%9.4f;" % (compare_temp[len(branch_list)+1].value[0]))
    fidsolution11.write('\n')
    fidsolution11.close()
    
    
def total_branch12(m=None,mpc=None,initial=None):
    global Lower_bound
    global Upper_bound
...
#            m.Equation(pd[i] == 0)
    ...
    #     pd_temp2[i]=pd[i].value[0]
    for i in range(0,len(Load_index_list)):
        pd_temp312[i]=Input[i].value[0]
        
    pd_save12.append(pd_temp312)

        
    # print(Slack_Pg_violation[0].value)
    print(compare_temp[len(branch_list)+1].value[0])
    
    FINAL12.append(compare_temp[len(branch_list)+1].value[0])
    # Final.append(compare_temp[len(branch_list)-1].value[0])
    solver12=time.time()-start
    print(solver12)
    solver_time12.append(solver12)
    
    filenamesolver12= dir+'s-solvertime12.txt'
    fidsolver12=open(filenamesolver12,'at')
    fidsolver12.write("%9.4f;" % (solver12))
    fidsolver12.write('\n')
    fidsolver12.close()
    
    filenamesolution12= dir+'s-total30_all-12.txt'
    fidsolution12=open(filenamesolution12,'at')
    fidsolution12.write("%9.4f;" % (compare_temp[len(branch_list)+1].value[0]))
    fidsolution12.write('\n')
    fidsolution12.close()
    
def total_branch13(m=None,mpc=None,initial=None):
    global Lower_bound
    global Upper_bound
...
#            m.Equation(pd[i] == 0)
    ...
    #     pd_temp2[i]=pd[i].value[0]
    for i in range(0,len(Load_index_list)):
        pd_temp313[i]=Input[i].value[0]
        
    pd_save13.append(pd_temp313)

        
    # print(Slack_Pg_violation[0].value)
    print(compare_temp[len(branch_list)+1].value[0])
    
    FINAL13.append(compare_temp[len(branch_list)+1].value[0])
    # Final.append(compare_temp[len(branch_list)-1].value[0])
    solver13=time.time()-start
    print(solver13)
    solver_time13.append(solver13)
    
    filenamesolver13= dir+'s-solvertime13.txt'
    fidsolver13=open(filenamesolver13,'at')
    fidsolver13.write("%9.4f;" % (solver13))
    fidsolver13.write('\n')
    fidsolver13.close()
    
    filenamesolution13= dir+'s-total30_all-13.txt'
    fidsolution13=open(filenamesolution13,'at')
    fidsolution13.write("%9.4f;" % (compare_temp[len(branch_list)+1].value[0]))
    fidsolution13.write('\n')
    fidsolution13.close()
    
    
    
def total_branch14(m=None,mpc=None,initial=None):
    global Lower_bound
    global Upper_bound
...
#            m.Equation(pd[i] == 0)
    ...
    #     pd_temp2[i]=pd[i].value[0]
    for i in range(0,len(Load_index_list)):
        pd_temp314[i]=Input[i].value[0]
        
    pd_save14.append(pd_temp314)

        
    # print(Slack_Pg_violation[0].value)
    print(compare_temp[len(branch_list)+1].value[0])
    
    FINAL14.append(compare_temp[len(branch_list)+1].value[0])
    # Final.append(compare_temp[len(branch_list)-1].value[0])
    solver14=time.time()-start
    print(solver14)
    solver_time14.append(solver14)
    
    filenamesolver14= dir+'s-solvertime14.txt'
    fidsolver14=open(filenamesolver14,'at')
    fidsolver14.write("%9.4f;" % (solver14))
    fidsolver14.write('\n')
    fidsolver14.close()
    
    filenamesolution14= dir+'s-total30_all-14.txt'
    fidsolution14=open(filenamesolution14,'at')
    fidsolution14.write("%9.4f;" % (compare_temp[len(branch_list)+1].value[0]))
    fidsolution14.write('\n')
    fidsolution14.close()
...
if __name__ == '__main__':
    ...
    #    global Bus_admittance_full_1
        ...
        pd_temp38= np.zeros((len(pd_temp0),),dtype='float32')
        pd_temp39= np.zeros((len(pd_temp0),),dtype='float32')
        pd_temp310= np.zeros((len(pd_temp0),),dtype='float32')
        pd_temp311= np.zeros((len(pd_temp0),),dtype='float32')
        pd_temp312= np.zeros((len(pd_temp0),),dtype='float32')
        pd_temp313= np.zeros((len(pd_temp0),),dtype='float32')
        pd_temp314= np.zeros((len(pd_temp0),),dtype='float32')
        ...
        load8=pd_temp0*3/10
        load9=pd_temp0*2/10
        load10=pd_temp0*1/10
        load11=pd_temp0*0
        load12=-pd_temp0*1/10
        load13=-pd_temp0*2/10
        load14=-pd_temp0*3/10
        ...
        m8 = GEKKO(remote=False)
        m8.options.SOLVER = 1
        m8.options.MAX_ITER = 1000
        m9 = GEKKO(remote=False)
        m9.options.SOLVER = 1
        m9.options.MAX_ITER = 1000
        m10 = GEKKO(remote=False)
        m10.options.SOLVER = 1
        m10.options.MAX_ITER = 1000
        m11 = GEKKO(remote=False)
        m11.options.SOLVER = 1
        m11.options.MAX_ITER = 1000
        m12 = GEKKO(remote=False)
        m12.options.SOLVER = 1
        m12.options.MAX_ITER = 1000
        m13 = GEKKO(remote=False)
        m13.options.SOLVER = 1
        m13.options.MAX_ITER = 1000
        m14 = GEKKO(remote=False)
        m14.options.SOLVER = 1
        m14.options.MAX_ITER = 1000
        
        # mpc = loadcase('case118_rev_100.py')
        threads = []
        
        
        t8 = threading.Thread(target=total_branch8, args=(m8, mpc, load8))
        threads.append(t8)
        t8.start()
        t9 = threading.Thread(target=total_branch9, args=(m9, mpc, load9))
        threads.append(t9)
        t9.start()
        t10 = threading.Thread(target=total_branch10, args=(m10, mpc, load10))
        threads.append(t10)
        t10.start()
        t11 = threading.Thread(target=total_branch11, args=(m11, mpc, load11))
        threads.append(t11)
        t11.start()
        t12 = threading.Thread(target=total_branch12, args=(m12, mpc, load12))
        threads.append(t12)
        t12.start()
        t13 = threading.Thread(target=total_branch13, args=(m13, mpc, load13))
        threads.append(t13)
        t13.start()
        t14 = threading.Thread(target=total_branch14, args=(m14, mpc, load14))
        threads.append(t14)
        t14.start()
        ...
        if len(FINAL8)<IRound+1:
            FINAL8.append(-100)
            filenamesolver8= dir+'s-solvertime8.txt'
            fidsolver8=open(filenamesolver8,'at')
            fidsolver8.write("%9.4f;" % (1000000))
            fidsolver8.write('\n')
            fidsolver8.close()
    
            filenamesolution8= dir+'s-total30_all-8.txt'
            fidsolution8=open(filenamesolution8,'at')
            fidsolution8.write("%9.4f;" % (-100))
            fidsolution8.write('\n')
            fidsolution8.close()
            
            pd_save8.append(pd_temp0)
            
        if len(FINAL9)<IRound+1:
            FINAL9.append(-100)
            filenamesolver9= dir+'s-solvertime9.txt'
            fidsolver9=open(filenamesolver9,'at')
            fidsolver9.write("%9.4f;" % (1000000))
            fidsolver9.write('\n')
            fidsolver9.close()
    
            filenamesolution9= dir+'s-total30_all-9.txt'
            fidsolution9=open(filenamesolution9,'at')
            fidsolution9.write("%9.4f;" % (-100))
            fidsolution9.write('\n')
            fidsolution9.close()
            
            pd_save9.append(pd_temp0)
            
        if len(FINAL10)<IRound+1:
            FINAL10.append(-100)
            filenamesolver10= dir+'s-solvertime10.txt'
            fidsolver10=open(filenamesolver10,'at')
            fidsolver10.write("%9.4f;" % (1000000))
            fidsolver10.write('\n')
            fidsolver10.close()
    
            filenamesolution10= dir+'s-total30_all-10.txt'
            fidsolution10=open(filenamesolution10,'at')
            fidsolution10.write("%9.4f;" % (-100))
            fidsolution10.write('\n')
            fidsolution10.close()
            
            pd_save10.append(pd_temp0)
            
        if len(FINAL11)<IRound+1:
            FINAL11.append(-100)
            filenamesolver11= dir+'s-solvertime11.txt'
            fidsolver11=open(filenamesolver11,'at')
            fidsolver11.write("%9.4f;" % (1000000))
            fidsolver11.write('\n')
            fidsolver11.close()
    
            filenamesolution11= dir+'s-total30_all-11.txt'
            fidsolution11=open(filenamesolution11,'at')
            fidsolution11.write("%9.4f;" % (-100))
            fidsolution11.write('\n')
            fidsolution11.close()
            
            pd_save11.append(pd_temp0)
            
        if len(FINAL12)<IRound+1:
            FINAL12.append(-100)
            filenamesolver12= dir+'s-solvertime12.txt'
            fidsolver12=open(filenamesolver12,'at')
            fidsolver12.write("%9.4f;" % (1000000))
            fidsolver12.write('\n')
            fidsolver12.close()
    
            filenamesolution12= dir+'s-total30_all-12.txt'
            fidsolution12=open(filenamesolution12,'at')
            fidsolution12.write("%9.4f;" % (-100))
            fidsolution12.write('\n')
            fidsolution12.close()
            
            pd_save12.append(pd_temp0)
            
        if len(FINAL13)<IRound+1:
            FINAL13.append(-100)
            filenamesolver13= dir+'s-solvertime13.txt'
            fidsolver13=open(filenamesolver13,'at')
            fidsolver13.write("%9.4f;" % (1000000))
            fidsolver13.write('\n')
            fidsolver13.close()
    
            filenamesolution13= dir+'s-total30_all-13.txt'
            fidsolution13=open(filenamesolution13,'at')
            fidsolution13.write("%9.4f;" % (-100))
            fidsolution13.write('\n')
            fidsolution13.close()
            
            pd_save13.append(pd_temp0)
            
        if len(FINAL14)<IRound+1:
            FINAL14.append(-100)
            filenamesolver14= dir+'s-solvertime14.txt'
            fidsolver14=open(filenamesolver14,'at')
            fidsolver14.write("%9.4f;" % (1000000))
            fidsolver14.write('\n')
            fidsolver14.close()
    
            filenamesolution14= dir+'s-total30_all-14.txt'
            fidsolution14=open(filenamesolution14,'at')
            fidsolution14.write("%9.4f;" % (-100))
            fidsolution14.write('\n')
            fidsolution14.close()
            
            pd_save14.append(pd_temp0)
        ...
        P_D_normalization.append(pd_save8[IRound])
        P_D_normalization.append(pd_save9[IRound])
        P_D_normalization.append(pd_save10[IRound])
        P_D_normalization.append(pd_save11[IRound])
        P_D_normalization.append(pd_save12[IRound])
        P_D_normalization.append(pd_save13[IRound])
        P_D_normalization.append(pd_save14[IRound])
        
        
        dir='./S-TEST/'
        for i in range(0,7):
            np.savetxt(dir+'s-TEST_'+str(i+IRound*42+7)+'.txt', np.array(P_D_normalization[len(P_D_normalization)-i-1]), fmt="%s")
        ...
        dir = './S-SOLVE_TIME/'
        filenametime2= dir+'time2.txt'
        fidtime2=open(filenametime2,'at')
        fidtime2.write("%9.4f;" % (FINISH))
        fidtime2.write('\n')
        fidtime2.close()
