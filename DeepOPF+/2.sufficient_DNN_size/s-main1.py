...
if __name__ == '__main__':
    ...
    #    global Bus_admittance_full_1
        ...
        COMPARE=[]
        indexxx=0
        for kkk in range(0,int(len(P_D_normalization)/4)):
        ...
        dir='./S-COMPARE/' 
        np.savetxt(dir+'compare1.txt', np.array(COMPARE), fmt="%s")
        np.savetxt(dir+'Finish_1_'+str(IRound)+'.txt', np.array([1]), fmt="%s")
        ...
        for wait in range(0,1000):  
            Temp=0
            dir = './S-COMPARE/'
            if os.access(dir+'Finish_1_'+str(IRound)+'.txt', os.F_OK):
                Temp=Temp+1
            if os.access(dir+'Finish_2_'+str(IRound)+'.txt', os.F_OK):
                Temp=Temp+1
            if os.access(dir+'Finish_3_'+str(IRound)+'.txt', os.F_OK):
                Temp=Temp+1                
            if os.access(dir+'Finish_4_'+str(IRound)+'.txt', os.F_OK):
                Temp=Temp+1
            if Temp==4:
                time.sleep(0)
            else:
        ...
        while line:
            ...
            line = f.readline()
        f.close()
        compare1 = np.array(data_list)
        compare1 = compare1.reshape(len(compare1),).tolist()
        f = open(dir+'compare2.txt')
        line = f.readline()
        data_list = []
        while line:
            num = list(map(float,line.split()))
            ...
            line = f.readline()
        f.close()
        compare2 = np.array(data_list)
        compare2 = compare2.reshape(len(compare2),).tolist()
        f = open(dir+'compare3.txt')
        line = f.readline()
        data_list = []
        while line:
            num = list(map(float,line.split()))
            ...
            line = f.readline()
        f.close()
        compare3 = np.array(data_list)
        compare3 = compare3.reshape(len(compare3),).tolist()
        f = open(dir+'compare4.txt')
        line = f.readline()
        data_list = []
        while line:
            num = list(map(float,line.split()))
            ...
            line = f.readline()
        f.close()
        compare4 = np.array(data_list)
        compare4 = compare4.reshape(len(compare4),).tolist()
		
        COMPARE=compare1+compare2+compare3+compare4
                                   
        
        NUMBER_PD=len(P_D_normalization)
        ...
        # fidin.write("%9.4f;" % (increase2))
        fidin.write("%9.4f;" % (increase3))
        fidin.write('\n')
        fidin.close()
        
        
        for kkk in range(NUMBER_PD,len(P_D_normalization)):
        ...
        if (r['success']== True):
            filenamesuccess= 's-TEST_success.txt'
        ...
        model=nn.Sequential(
        nn.Linear(Load_index.shape[0], 4*8),
        nn.ReLU(),
        nn.Linear(4*8, 2*8),
        nn.ReLU(),
        nn.Linear(2*8, 8),
        nn.ReLU(),
        nn.Linear(8, Gen_index.shape[0]),
        nn.Sigmoid(),)
        
        optimizer = optim.SGD(model.parameters(), lr=0.01, momentum=0.9)
        penalty = Penalty()
        # model.load_state_dict(torch.load('PreDCOPF_case118_0.93_dnn0106-10.pth',map_location='cpu'))
        # model[0].weight.data = torch.from_numpy(weight_matrix[0])
        
        for i in range(0, len(layers)-1):
            model[2*i].weight.data = torch.from_numpy(weight_matrix[i]).float()
            model[2*i].bias.data = torch.from_numpy(bias_vector[i]).float()
        
        
        
        NNOutput=model(torch.from_numpy(Input).float())
            
        Pg=(torch.from_numpy(Power_Upbound_Gen)-torch.from_numpy(Power_Lowbound_Gen))*NNOutput+torch.from_numpy(Power_Lowbound_Gen)
        Pg=torch.reshape(Pg,(Pg.shape[0],1))
        ...
        pd=pd.astype(np.float32)
            ...
            # Branch_flow=(np.dot(PTDF_pg_exceptslack,Pg.detach().numpy()).squeeze()+(sum(pd)-sum(Pg.detach().numpy()))*PTDF_pg_slack-np.dot(PTDF_ori,pd))/Power_bound_Branch
            
            # A=(torch.from_numpy(PTDF_pg_exceptslack).mm(Pg)).squeeze()
        Branch_flow=torch.div(((torch.from_numpy(PTDF_pg_exceptslack).mm(Pg)).squeeze()+((np.sum(pd))-torch.sum(Pg))*torch.from_numpy(PTDF_pg_slack)-torch.from_numpy(np.dot(PTDF_ori,pd))),torch.from_numpy(Power_bound_Branch)).float()
        Slack0=torch.div((np.sum(pd)-torch.sum(Pg)),Power_Upbound_Gen_Slack).expand(1).float()
        Slack1=torch.div((torch.sum(Pg)-np.sum(pd)),Power_Upbound_Gen_Slack).expand(1).float()
        # kkk=torch.cat((Slack0, Slack1))
        VL=torch.cat((Branch_flow,Slack0, Slack1), 0)
            # Branch_flow=torch.div(Branch_flow,torch.from_numpy(Power_bound_Branch))
        loss=penalty(VL)
        ...
        Gradient_bias=[];
        for i in range(0, len(layers)-1):
            Gradient_weight=np.hstack((Gradient_weight,grads[i*2].detach().numpy()))
            Gradient_bias=np.hstack((Gradient_bias,grads[i*2+1].detach().numpy()))
            # Gradient_weight.append((Final_temp[0]-Final[0])/epsilon)
        WEIGHT2.append((weight-alpha*(Gradient_weight).astype(np.float32)))
        BIAS2.append((bias-alpha*(Gradient_bias)).astype(np.float32))
        print("Finish update weight and bias")
        time_update=time.time()-start2
        P_update.append(time_update)
        print(time_update)
        
        
        weight=WEIGHT2[IRound]
        bias=BIAS2[IRound]
        ...
        for i in params:
            ...
            if len(list(i.size()))==2:
                ...
                for j in range(0,dim1):
                    ...
                    bias_index += 1
        dir = './S-PTH/'
        torch.save(model.state_dict(), dir+'s-TEST_'+str(IRound+1)+'.pth')
        
        torch.manual_seed(1)    # reproducible

        if FFALL[IRound]<=0  or IRound==399 :
