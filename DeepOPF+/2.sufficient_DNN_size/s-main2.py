...
if __name__ == '__main__':
    ...
    #    global Bus_admittance_full_1
        ...
        COMPARE=[]
        indexxx=0
        for kkk in range(int(len(P_D_normalization)/4),2*int(len(P_D_normalization)/4)):
        ...
        dir='./S-COMPARE/' 
        np.savetxt(dir+'compare2.txt', np.array(COMPARE), fmt="%s")
        np.savetxt(dir+'Finish_2_'+str(IRound)+'.txt', np.array([1]), fmt="%s")
