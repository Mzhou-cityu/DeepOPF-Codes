...
if __name__ == '__main__':
    ...
    #    global Bus_admittance_full_1
        ...
        COMPARE=[]
        indexxx=0
        for kkk in range(3*int(len(P_D_normalization)/4),len(P_D_normalization)):
        ...
        dir='./S-COMPARE/' 
        np.savetxt(dir+'compare4.txt', np.array(COMPARE), fmt="%s")
        np.savetxt(dir+'Finish_4_'+str(IRound)+'.txt', np.array([1]), fmt="%s")
