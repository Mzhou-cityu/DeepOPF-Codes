...
class Penalty_ACPF(Function):
    ...
    def forward(ctx, NN_input, load):
        ...
        for i in range(NN_input_np.shape[0]):
            #PG      
            Pred_PG_train = PG_Lowbound_Active+(PG_Upbound_Active-PG_Lowbound_Active)*NN_input_np[i,0:PG_Lowbound_Active.shape[1]]
            #Slack_bus_Voltage
            Pred_Slack_V_train = Lowbound_Slack_V+(Upbound_Slack_V-Lowbound_Slack_V)*NN_input_np[i,PG_Lowbound_Active.shape[1]]
            #Voltage
            Pred_Volt_train = Lowbound_Volt+(Upbound_Volt-Lowbound_Volt)*NN_input_np[i,PG_Lowbound_Active.shape[1]+1:NN_output_number]
...
#            Q_D_temp_train =  train_x_np[i,NN_input_number:2*NN_input_number]                   
            ...
            mpc_pf_train["gen"][Gen_index_list,5] = Pred_Volt_train[:]
            #slack vg
            mpc_pf_train["gen"][Slack_GEN_index_list,5] = Pred_Slack_V_train[:]

            #run power flow
            ppopt = ppoption()
            ppopt = ppoption(ppopt, OUT_ALL=0, VERBOSE=False, ENFORCE_Q_LIMS=True)         
...
#            
#            NN_input_modified_temp = NN_input_np
...
#            #PG      
#            Pred_PG_train_plus_h = PG_Lowbound+(PG_Upbound-PG_Lowbound)*NN_input_modified_temp[i,0:int(NN_output_number/2)]
#            #Slack_bus_Voltage
#            Pred_Slack_V_train_plus_h = Lowbound_Slack_V+(Upbound_Slack_V-Lowbound_Slack_V)*NN_input_modified_temp[i,int((NN_output_number-1)/2)]
#            #Voltage
#            Pred_Volt_train_plus_h = Lowbound_Volt+(Upbound_Volt-Lowbound_Volt)*NN_input_modified_temp[i,int((NN_output_number-1)/2)+1:NN_output_number]                 
#            train_x_np = load.numpy()
#            P_D_temp_train =  (P_D_train_std+1e-8)*train_x_np[i,0:NN_input_number]+P_D_train_mean
#            Q_D_temp_train =  (Q_D_train_std+1e-8)*train_x_np[i,NN_input_number:2*NN_input_number]+(Q_D_train_mean)                                         
#            #solving PF
#            mpc_pf_train_puls_h = loadcase('case30_rev.py')
...
#                    mpc_pf_train_puls_h["gen"][index,1] =  Pred_PG_train_plus_h[0,count]*standval
#                    mpc_pf_train_puls_h["gen"][index,5] =  Pred_Volt_train_plus_h[count]
#                    count = count + 1                       
#                else:
#                    mpc_pf_train_puls_h["gen"][index,5] =  Pred_Slack_V_train_plus_h[0,0]
#            #run power flow
#            r1_plus_h_pf = runpf(mpc_pf_train_puls_h)
#            penalty_plus_h = zero_order_penalty_abs(r1_plus_h_pf)                    
#            #################################################################################################################
#            #estimate gradient
#            NN_input_modified_temp[i,:] = Original_value - vector1*h         
#            #PG      
#            Pred_PG_train_minus_h = PG_Lowbound+(PG_Upbound-PG_Lowbound)*NN_input_modified_temp[i,0:int(NN_output_number/2)]             
#            #Slack_bus_Voltage
#            Pred_Slack_V_train_minus_h = Lowbound_Slack_V+(Upbound_Slack_V-Lowbound_Slack_V)*NN_input_modified_temp[i,int((NN_output_number-1)/2)]
#            #Voltage
#            Pred_Volt_train_minus_h = Lowbound_Volt+(Upbound_Volt-Lowbound_Volt)*NN_input_modified_temp[i,int((NN_output_number-1)/2)+1:NN_output_number]                    
#            #solving PF
#            mpc_pf_train_minus_h = loadcase('case30_rev.py')
#                
#            count = 0
#            [m_bus,n_bus]=mpc_pf_train_minus_h["bus"].shape
#            for index in range(0,m_bus):
#                if mpc_pf_train_minus_h["bus"][index,2] != 0 and mpc_pf_train_minus_h["bus"][index,3] != 0:
...
#                    mpc_pf_train_minus_h["gen"][index,1] =  Pred_PG_train_minus_h[0,count]*standval
#                    mpc_pf_train_minus_h["gen"][index,5] =  Pred_Volt_train_minus_h[count]
#                    count = count + 1                       
#                else:
#                    mpc_pf_train_minus_h["gen"][index,5] =  Pred_Slack_V_train_minus_h[0,0]
#            #run power flow
#            r1_minus_h_pf = runpf(mpc_pf_train_minus_h)
...
#    Sbus = makeSbus(baseMVA, ppc["bus"], ppc["gen"])
#
#    Bp, Bpp = makeB(baseMVA, bus, branch, alg)
#    V, success, _ = fdpf(Ybus, Sbus, V0, Bp, Bpp, ref, pv, pq, ppopt)
#    ## update data matrices with solution
#    bus_temp, gen_temp, branch_temp = pfsoln(baseMVA, bus, gen, branch, Ybus, Yf, Yt, V, ref, pv, pq)
#
#    ##-----  output results  -----
#    ## convert back to original bus numbering & print results
#    ppc["bus"], ppc["gen"], ppc["branch"] = bus_temp, gen_temp, branch_temp
#
#    results = int2ext(ppc)
#
...
#    #PV-PG_slack_bus
#    Pg_temp1 = pf_results[0]["gen"][:,9]-pf_results[0]["gen"][:,1]
#    Pg_temp1[Pg_temp1 < 0]=0
#    Pg_temp2 = pf_results[0]["gen"][:,1]-pf_results[0]["gen"][:,8]
#    Pg_temp2[Pg_temp2 < 0]=0   
#    PG_penalty = np.abs(Pg_temp1)+np.abs(Pg_temp2)
#    #PV-QG
#    Qg_temp1 = pf_results[0]["gen"][:,4]-pf_results[0]["gen"][:,2]
#    Qg_temp1[Qg_temp1 < 0] = 0
#    Qg_temp2 = pf_results[0]["gen"][:,2]-pf_results[0]["gen"][:,3]
#    Qg_temp2[Qg_temp2 < 0] = 0
#    QG_penalty = np.abs(Qg_temp1)+np.abs(Qg_temp2)
##    #PQ-V
#    PQ_index = np.where(pf_results[0]["bus"][:,1]==1)[0]
#    V_temp1 = pf_results[0]["bus"][PQ_index,12]-pf_results[0]["bus"][PQ_index,7]
...
#    Ft = abs(pf_results[0]["branch"][:, 15] + 1j * pf_results[0]["branch"][:, 16])
#    ctol = 5e-06
#    [m_branch,n_branch]=mpc["branch"].shape
#    Branch_index = np.where(pf_results[0]["branch"][:,5] != 0)[0]
#    Branch_bound = pf_results[0]["branch"][Branch_index, 5] - ctol
#    Ff_temp = Ff[Branch_index]-Branch_bound
#    Ff_temp[Ff_temp<0] = 0
#    Ft_temp = Ft[Branch_index]-Branch_bound
#    Ft_temp[Ft_temp<0] = 0    
#    Ff_penalty = np.abs(Ff_temp)
#    Ft_penalty = np.abs(Ft_temp)
##    
...
def zero_order_penalty(pf_results=None):
    #PV-PG_slack_bus
    PG_penalty = np.power(2*(pf_results[0]["gen"][0,1]-pf_results[0]["gen"][0,9])/(pf_results[0]["gen"][0,8]-pf_results[0]["gen"][0,9])-1,2)-1
    if PG_penalty < 0: PG_penalty = 0
    #PV-QG
    QG_penalty = np.power(2*(pf_results[0]["gen"][:,2]-pf_results[0]["gen"][:,4])/(pf_results[0]["gen"][:,3]-pf_results[0]["gen"][:,4])-1,2)-1
    QG_penalty[QG_penalty < 0] = 0
    #PQ-V
    PQ_index = np.where(pf_results[0]["bus"][:,1]==1)[0]
    V_penalty = np.power(2*(pf_results[0]["bus"][PQ_index,7]-pf_results[0]["bus"][PQ_index,12])/(pf_results[0]["bus"][PQ_index,11]-pf_results[0]["bus"][PQ_index,12])-1,2)-1
    V_penalty[V_penalty < 0] = 0
    #Branch
    Ff = abs(pf_results[0]["branch"][:, 13] + 1j * pf_results[0]["branch"][:, 14])
    Ft = abs(pf_results[0]["branch"][:, 15] + 1j * pf_results[0]["branch"][:, 16])
    ctol = 5e-06
    ...
    Branch_index = np.where(pf_results[0]["branch"][:,5] != 0)[0]
    Branch_bound = pf_results[0]["branch"][Branch_index, 5] - ctol
    Ff_penalty = np.power((Ff[Branch_index])/(Branch_bound),2)-1
    Ff_penalty[Ff_penalty < 0] = 0
    Ft_penalty = np.power((Ft[Branch_index])/(Branch_bound),2)-1
    Ft_penalty[Ft_penalty < 0] = 0                    
    if np.mean(Ff_penalty)+np.mean(Ft_penalty) != 0:
        Branch_list.append(1)
    return PG_penalty+np.mean(QG_penalty)+np.mean(V_penalty)+np.mean(Ff_penalty)+np.mean(Ft_penalty)
...
#        if (bus[j,2] != 0 or bus[j,3] != 0):
    ...
    for j in range(0,gen.shape[0],1):
        if Slack_bus!= gen[j,0]:
            Gen_index_list.append(int(j))
            PG_Upbound[0,gen_index] = mpc_gen[j,8]/standval
            PG_Lowbound[0,gen_index] = mpc_gen[j,9]/standval
            gen_index = gen_index +1
            if mpc_gen[j,8] == 0 and mpc_gen[j,9] == 0:
    ...
    for j in range(0,PG_Upbound_Active.shape[1]):
        PG_Upbound_Active[0,j] = mpc_gen[Active_GEN_index_list[j],8]/standval
        PG_Lowbound_Active[0,j] = mpc_gen[Active_GEN_index_list[j],9]/standval
    ...
    Lowbound_Volt = np.zeros((gen.shape[0]-1,),dtype='float32')
    Upbound_Volt = np.zeros((gen.shape[0]-1,),dtype='float32')
    Lowbound_Volt = Lowbound_Volt.T
    Upbound_Volt = Upbound_Volt.T
    index = 0
    for i in range(bus.shape[0]):
        if bus[i,1] == 2:
            Lowbound_Volt[index] = np.power(bus[i,12],1)
            Upbound_Volt[index] = np.power(bus[i,11],1)
            index = index + 1
        elif bus[i,1]!= 3:
...
if __name__ == '__main__':
    ...
    numOfdata = 0
    #reading data
    dir='/data/grad/px018/sampling_data/AC-OPF/PR2_ZO/30/typical_10/'
    #dir=''
    # for index in range(1, 2, 1):
    #     filename= dir+'ACOPF_load_case30_60000.txt'        
    #     with open(filename, 'r') as f:                              
    #         data = f.readlines()
    #         numOfdata = numOfdata + int(len(data)/3)
    #         j = 0
    #         for line in data:
    #                 line = line.strip('\n')
    #                 if j % 3 == 0:
    ...
    #                         if odom[i]:
    #                             P_D_list.append(float(odom[i].split(';')[1]))
    #                             Q_D_list.append(float(odom[i].split(';')[2]))
    #                 elif j % 3 == 1:
    #                     odom = line.split(',')  # load
    #                     for i in range(len(odom)):
    #                         if odom[i]:
    #                             P_G_list.append(float(odom[i].split(';')[0]))
    #                             Q_G_list.append(float(odom[i].split(';')[1]))
    #                             V_list.append(float(odom[i].split(';')[2]))
    #                 else:
    #                     odom = line.split(',')
    #                     if odom[0]:
    #                         Slack_V_list.append(float(odom[0]))
    #                 j = j + 1

    # del data,line,odom
    P_D = np.load("P_D.npy").astype('float32')
    Q_D = np.load("Q_D.npy").astype('float32')    
    V = np.load("V.npy").astype('float32')
    Slack_V = np.load("Slack_V.npy").astype('float32')
    P_G_all = np.load("P_G_all.npy").astype('float32')
    P_G = P_G_all[:,Active_GEN_index_list2PG_train]    
    Q_G = np.load("Q_G.npy").astype('float32')       
    
    #divide into 1000 per group
    segment1 = 60
    #generate data array
    num_of_group = 1000
    segment1 = int(P_D.shape[0]/num_of_group)
    # P_D = np.array(P_D_list, dtype=np.float32)
    # P_D = P_D.reshape(-1, NN_input_number)
    # Q_D = np.array(Q_D_list, dtype=np.float32)
    # Q_D = Q_D.reshape(-1, NN_input_number)    
    # V = np.array(V_list, dtype=np.float32)
    # V = V.reshape(-1, All_Gen_number-1)
    # Slack_V = np.array(Slack_V_list, dtype=np.float32)
    # Slack_V = Slack_V.reshape(-1, 1)
    # P_G_all = np.array(P_G_list, dtype=np.float32)
    # P_G_all = P_G_all.reshape(numOfdata, All_Gen_number-1)
    # P_G = P_G_all[:,Active_GEN_index_list2PG_train]
    # Q_G = np.array(Q_G_list, dtype=np.float32)
    # Q_G = Q_G.reshape(numOfdata, All_Gen_number-1)
    ############################################## 
    # Preprocessing training data
    segment = int(training_amout/1000)
    P_D_train = P_D[0:num_of_group * (segment-1)]
    ...
    Q_D_train = Q_D[0:num_of_group * (segment-1)]
    ...
    P_G_train = P_G[0:num_of_group * (segment-1)]/standval
    ...
    Volt_train = V[0:num_of_group * (segment-1)]
    ...
    Slack_V_train = Slack_V[0:num_of_group * (segment-1)]
    Lowbound_Slack_V_training = np.tile(Lowbound_Slack_V,(len(Q_D_train),1)) 
    Upbound_Slack_V_training= np.tile(Upbound_Slack_V,(len(Q_D_train),1))
...
#    P_D_val=P_D[num_of_group*(segment-1):np.size(P_D,0)]
#    Q_D_val=Q_D[num_of_group*(segment-1):np.size(P_D,0)]    
#    P_D_val_normolization=(P_D_val)
#    Q_D_val_normolization=(Q_D_val) 
#    #if do pre-processing
#    P_D_val_normolization=(P_D_val-P_D_train_mean)/(P_D_train_std+1e-8)
#    Q_D_val_normolization=(Q_D_val-Q_D_train_mean)/(Q_D_train_std+1e-8)    
#    D_val_normolization=np.concatenate((P_D_val_normolization,Q_D_val_normolization),axis=1)
#
#    P_G_val=P_G[num_of_group*(segment-1):np.size(P_D,0)]/standval
#    Lowbound_PG_val=np.tile(PG_Lowbound_Active,(len(P_G_val),1))
#    Upbound_PG_val=np.tile(PG_Upbound_Active,(len(P_G_val),1))
#    PG_val_normolization=(P_G_val-Lowbound_PG_val)/(Upbound_PG_val-Lowbound_PG_val)
#
#    Volt_val=V[num_of_group*(segment-1):np.size(P_D,0)]
#    Lowbound_Volt_val=np.tile(Lowbound_Volt,(len(Volt_val),1))
#    Upbound_Volt_val=np.tile(Upbound_Volt,(len(Volt_val),1))
#    Volt_val_normolization=(Volt_val-Lowbound_Volt_val)/(Upbound_Volt_val-Lowbound_Volt_val)
#
#    Slack_V_val=Slack_V[num_of_group*(segment-1):np.size(P_D,0)]
#    Lowbound_Slack_V_val=np.tile(Lowbound_Slack_V,(len(P_G_val),1))
#    Upbound_Slack_V_val=np.tile(Upbound_Slack_V,(len(P_G_val),1))
#    Slack_V_val_normolization=(Slack_V_val-Lowbound_Slack_V_val)/(Upbound_Slack_V_val-Lowbound_Slack_V_val)
#
#    PV_val_temp=np.concatenate((PG_val_normolization,Slack_V_val_normolization),axis=1)
#    PV_val_normolization=np.concatenate((PV_val_temp,Volt_val_normolization),axis=1)
...
#    Q_D_train_normolization = (Q_D_train - Q_D_train_mean) / (Q_D_train_std+1e-8)
    ...
    Slack_V_test = Slack_V[start_idx:P_D.shape[0]]
    Lowbound_Slack_V_test = np.tile(Lowbound_Slack_V,(len(Q_D_test),1)) 
    Upbound_Slack_V_test = np.tile(Upbound_Slack_V,(len(Q_D_test),1))
...
#    criterion = nn.MSELoss(reduce=True,size_average = True)
    ...
    model.load_state_dict(torch.load('ACOPF_case_30_dnn_200_reference.pth',map_location='cpu'))
    # Test dataset
...
#        fid1.write("%9.5f;%9.5f;%9.5f\n" % (cost_list[i],dnn_time[i],cplex_time[i]))
    fid1.close()
#    filename1='1_0_feasible.txt'
#    fid1=open(filename1,'a')
#    for i in range(0,len(infeasible_branch_index_list)):
#        fid1.write("%9.5f;%9.5f;%9.5f\n;%9.5f\n;%9.5f\n;%9.5f\n" % (infeasible_branch_index_list[i],infeasible_branch_number_list[i],infeasible_qg_index_list[i],infeasible_qg_number_list[i],infeasible_volt_index_list[i],infeasible_volt_number_list[i]))
##        fid1.write("%9.5f;%9.5f;%9.5f\n" % (cost_list[i],dnn_time[i],cplex_time[i]))
#    fid1.close()
    feasible_number_list =np.array(feasible_number_list)
    np.savetxt("feasible_list1_0.txt", feasible_number_list)    
#    np.savetxt("a1_0.txt", KK)
#    Pre_pg_list =np.array(Pre_pg_list).reshape(-1,5)
#    Pre_slack_list =np.array(Pre_slack_list).reshape(-1,1)
#    Pre_voltage_list =np.array(Pre_voltage_list).reshape(-1,5)
#    np.savetxt("pg1_0.txt", Pre_pg_list)
#    np.savetxt("voltage1_0.txt", Pre_voltage_list)
#    np.savetxt("slack1_0.txt", Pre_slack_list)
...
#    percentage_genpg=np.array(percentage_genpg).reshape(-1,4)
#    percentage_genqg=np.array(percentage_genqg).reshape(-1,4)
#    percentage_branch1=np.array(percentage_branch1).reshape(-1,4)
#    percentage_branch2=np.array(percentage_branch2).reshape(-1,4)
#    percentage_voltage=np.array(percentage_voltage).reshape(-1,4)
#    OPFbranch1=np.array(OPFbranch1).reshape(-1,41)
#    OPFbranch2=np.array(OPFbranch2).reshape(-1,41)
#    OPFvoltage=np.array(OPFvoltage).reshape(-1,30)
#    OPFgenpg=np.array(OPFgenpg).reshape(-1,6)
#    OPFgenqg=np.array(OPFgenqg).reshape(-1,6)
#    np.savetxt("OPF_genpg_1_0.txt", OPFgenpg)
#    np.savetxt("OPF_genqg_1_0.txt", OPFgenqg)
#    np.savetxt("OPF_branch1_1_0.txt", OPFbranch1)
#    np.savetxt("OPF_branch2_1_0.txt", OPFbranch2)
#    np.savetxt("OPF_voltage_1_0.txt", OPFvoltage)
#    np.savetxt("infeasible_genpg_1_0.txt", percentage_genpg)
#    np.savetxt("infeasible_genqg_1_0.txt", percentage_genqg)
#    np.savetxt("infeasible_branch1_1_0.txt", percentage_branch1)
#    np.savetxt("infeasible_branch2_1_0.txt", percentage_branch2)
#    np.savetxt("infeasible_voltage_1_0.txt", percentage_voltage)